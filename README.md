## OoLibrary 
一个图书管理工具

### Reference
[Python recursive folder read](https://stackoverflow.com/questions/2212643/python-recursive-folder-read)  
[How to get an absolute file path in python](https://stackoverflow.com/questions/51520/how-to-get-an-absolute-file-path-in-python)  
[Revert deleted file in IDEA](https://blog.jetbrains.com/idea/2008/01/using-local-history-to-restore-deleted-files/)  
[Python re.sub](https://www.crifan.com/python_re_sub_detailed_introduction/)  
[Extracting extension from filename in python](https://stackoverflow.com/questions/541390/extracting-extension-from-filename-in-python/)  
[Filter punctuation with python](http://blog.csdn.net/mach_learn/article/details/41744487)  

